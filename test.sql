...
INSERT INTO users (name, role, age, state) VALUES('CSE','owner',35,'Alabama');
INSERT INTO users (name, role, age, state) VALUES('David','customer',33,'Alaska');
INSERT INTO users (name, role, age, state) VALUES('Floyd','customer',27,'Arizona');
INSERT INTO users (name, role, age, state) VALUES('James','customer',55,'Arkansas');
INSERT INTO users (name, role, age, state) VALUES('Ross','customer',24,'California');
INSERT INTO users (name, role, age, state) VALUES('Tyler','customer',19,'Colorado');
INSERT INTO users (name, role, age, state) VALUES('Neeren', 'customer',21,'Georgia');
...
select state from users group by state order by state asc limit 20
select u2.id, u.state from (
select state from users group by state order by state asc limit 20
) as u left outer join users u2 on u2.state=u.state order by u.state

select state from users where age>12 and age<= 45 group by state order by state asc limit 20

select u2.id, u.state from (
select state from users where age>12 and age<= 45 group by state order by state asc limit 20
) as u left outer join users u2 on u2.state=u.state order by u.state

select state from users where state='Alabama' group by state order by state asc limit 20

select u2.id, u.state from (
select state from users where state='Alabama' group by state order by state asc limit 20
) as u left outer join users u2 on u2.state=u.state order by u.state

select state from users where age>18 and age<=45 
group by state order by state asc limit 20
...
insert into us_t (id, state) select u2.id, u.state from (
select state from users group by state order by state asc limit 20
) as u left outer join users u2 on u2.state=u.state order by u.state
...
select s.pid, sum(s.quantity*s.price) from ps_t p, sales s, users u
where s.pid=p.id  and s.uid=u.id and u.state='Arkansas'  group by s.pid;